...
@file: downMonitor_ios.py
@time: 2024/11/22
@desc: 适配ios的pythonista版本。
        bilibili视频下载脚本，支持bvid下载，支持bark推送消息。
        b站up最新合集视频监控。仅下载今天和昨天内容，且不会重复下载，保存在自定义文件夹下。
        根据bvid列表（合集）进行下载，填写bvidlist、savepath、key变量即可
        savepath是pythonista的沙盒路径，需要手动创建文件夹
"""
...
# 配置，自定义
bvidlist = ["BVxxxx", ] # 需要下载的bvid列表
savepath = "~/Documents/downloads/bili"    # pythonista的路径，pythonista的路径是pythonista的沙盒路径，需要手动创建文件夹
...
def pythonistapath():
    # 获取Pythonista文档目录
    document_directory = os.path.expanduser(savepath)
    # 创建完整的文件路径
...
def get_save_path(filename):
    ...
    document_directory = os.path.expanduser(savepath)
    # 创建完整的文件路径
    return os.path.join(document_directory, filename)
...
def checkcid(cid):
    ...
    for filename in os.listdir(pythonistapath()):
